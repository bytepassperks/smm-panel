<?php
/**
 * City Landing Page - Kolkata
 * GEO SEO optimized for Kolkata, West Bengal
 */
...
$testimonials = [
    ['name' => 'Sayak B.', 'role' => 'YouTuber', 'text' => 'Best SMM service in Kolkata! Helped my Bengali channel grow to 50K subscribers.'],
    ['name' => 'Mithila S.', 'role' => 'Influencer', 'text' => 'Great service for Kolkata influencers. Fast delivery and amazing support team.'],
    ['name' => 'Arjun D.', 'role' => 'Digital Marketer', 'text' => 'Reliable SMM panel for my Kolkata agency clients. Competitive rates and quality service.']
];
?>
<!DOCTYPE html>
<html lang="en-IN">
<head>
    ...
    <meta name="keywords" content="SMM panel Kolkata, buy Instagram followers Kolkata, best SMM panel West Bengal">
    <meta name="robots" content="index, follow">
    <meta name="geo.region" content="IN-WB">
    <meta name="geo.placename" content="Kolkata">
    <link rel="canonical" href="<?= $siteUrl ?>/kolkata">
    <script type="application/ld+json">
    {"@context": "https://schema.org","@type": "LocalBusiness","name": "<?= htmlspecialchars($siteName) ?> - Kolkata","description": "Best SMM panel in Kolkata, West Bengal.","url": "<?= $siteUrl ?>/kolkata","areaServed": {"@type": "City", "name": "Kolkata"},"address": {"@type": "PostalAddress", "addressRegion": "West Bengal", "addressCountry": "IN"}}
    </script>
    ...
    <style>
        ...
        .city-hero { background: linear-gradient(135deg, #f12711 0%, #f5af19 100%); padding: 5rem 0; text-align: center; color: white; }
        ...
        .city-cta { background: linear-gradient(135deg, #f12711 0%, #f5af19 100%); padding: 4rem 0; text-align: center; color: white; }
        .city-cta h2 { font-size: 2rem; margin-bottom: 1rem; }
        .city-cta p { font-size: 1.1rem; opacity: 0.9; margin-bottom: 2rem; }
        .city-cta .btn { background: white; color: #f12711; padding: 14px 32px; font-size: 1.1rem; font-weight: 600; text-decoration: none; border-radius: 10px; }
...
<body>
    ...
    <section class="city-hero"><div class="city-hero-content"><div class="city-badge">📍 Best SMM Panel in Kolkata</div><h1>Grow Your Social Media in Kolkata</h1><p>Join thousands of creators and businesses in Kolkata who trust us for their social media growth.</p><div class="city-hero-btns"><a href="/register" class="btn btn-primary" style="background: white; color: #f12711;">Get Started Free</a><a href="/services" class="btn btn-secondary" style="border-color: white; color: white;">View Services</a></div></div></section>
    <section class="city-services"><h2 class="city-section-title">Our Services in Kolkata</h2><div class="services-city-grid"><?php foreach($services as $s): ?><div class="city-service-card"><h3><?= $s['name'] ?></h3><div class="price">Starting at <span><?= $s['starting'] ?></span></div><p><?= $s['desc'] ?></p></div><?php endforeach; ?></div></section>
    <section class="city-features"><h2 class="city-section-title">Why Choose Us in Kolkata?</h2><div class="features-city-grid"><?php foreach($features as $f): ?><div class="city-feature-card"><div class="city-feature-icon"><?= $f['icon'] ?></div><h3><?= $f['title'] ?></h3><p><?= $f['desc'] ?></p></div><?php endforeach; ?></div></section>
    <section class="city-testimonials"><h2 class="city-section-title">What Kolkata Customers Say</h2><?php foreach($testimonials as $t): ?><div class="city-testimonial"><p class="city-testimonial-text">"<?= $t['text'] ?>"</p><div class="city-testimonial-author"><strong><?= $t['name'] ?></strong><span>— <?= $t['role'] ?>, Kolkata</span></div></div><?php endforeach; ?></section>
    <section class="city-cta"><div class="container"><h2>Ready to Grow in Kolkata?</h2><p>Join thousands of satisfied customers across India.</p><a href="/register" class="btn">Create Free Account</a></div></section>
    <footer class="footer"><div class="container"><div class="footer-content"><div class="footer-section"><h4><?= htmlspecialchars($siteName) ?></h4><p>Best SMM panel in Kolkata and all across India.</p></div><div class="footer-section"><h4>Quick Links</h4><ul><li><a href="/services">Services</a></li><li><a href="/pricing">Pricing</a></li><li><a href="/blog">Blog</a></li></ul></div></div><div class="footer-bottom"><p>&copy; <?= date('Y') ?> <?= htmlspecialchars($siteName) ?>. All rights reserved.</p></div></div></footer>
